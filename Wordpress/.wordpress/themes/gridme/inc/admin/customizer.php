...
function gridme_register_theme_customizer( $wp_customize ) {

    if(method_exists('WP_Customize_Manager', 'add_panel')):
    $wp_customize->add_panel('gridme_main_options_panel', array( 'title' => esc_html__('Theme Options', 'gridme'), 'priority' => 10, ));
    endif;

    $wp_customize->get_section( 'title_tagline' )->panel = 'gridme_main_options_panel';
    $wp_customize->get_section( 'title_tagline' )->priority = 20;
    $wp_customize->get_section( 'header_image' )->panel = 'gridme_main_options_panel';
    $wp_customize->get_section( 'header_image' )->priority = 26;
    $wp_customize->get_section( 'background_image' )->panel = 'gridme_main_options_panel';
    $wp_customize->get_section( 'background_image' )->priority = 27;
    $wp_customize->get_section( 'colors' )->panel = 'gridme_main_options_panel';
    ...
    $wp_customize->get_control( 'background_color' )->description = esc_html__('To change Background Color, need to remove background image first:- go to Appearance : Customize : Theme Options : Background Image', 'gridme');

    if ( isset( $wp_customize->selective_refresh ) ) {
        $wp_customize->selective_refresh->add_partial( 'blogname', array(
            'selector'        => '.gridme-site-title a',
            'render_callback' => 'gridme_customize_partial_blogname',
        ) );
        $wp_customize->selective_refresh->add_partial( 'blogdescription', array(
            'selector'        => '.gridme-site-description',
            'render_callback' => 'gridme_customize_partial_blogdescription',
        ) );
    }

    gridme_getting_started($wp_customize);
    gridme_menu_options($wp_customize);
    gridme_header_options($wp_customize);
    gridme_posts_grid_options($wp_customize);
    gridme_post_options($wp_customize);
    gridme_navigation_options($wp_customize);
    gridme_page_options($wp_customize);
    gridme_social_profiles($wp_customize);
    gridme_footer_options($wp_customize);
    gridme_recomm_plugin_options($wp_customize);
    gridme_upgrade_to_pro($wp_customize);

}

add_action( 'customize_register', 'gridme_register_theme_customizer' );
 ...
 * @return void
 */
function gridme_customize_partial_blogname() {
    bloginfo( 'name' );
}
/**
 ...
 * @return void
 */
function gridme_customize_partial_blogdescription() {
    bloginfo( 'description' );
}

function gridme_customizer_js_scripts() {
    wp_enqueue_script('gridme-theme-customizer-js', get_template_directory_uri() . '/inc/admin/js/customizer.js', array( 'jquery', 'customize-preview' ), NULL, true);
}
add_action( 'customize_preview_init', 'gridme_customizer_js_scripts' );